...
export default function Planner() {
  ...
  const [schedulePlatformPicker, setSchedulePlatformPicker] = useState(null);
  ...
  useEffect(() => {
    ...
    if (state?.openSchedule && state?.scheduleContent !== undefined) {
      ...
      setSchedulePlatformPicker({ dateStr, dateTime });
  ...
  const openScheduleModal = (day) => {
    ...
    if (dayPosts.length > 0 || isPast) {
      ...
      setSchedulePlatformPicker({ dateStr, dateTime: `${dateStr}T09:00` });
    }
  };
  ...
  const handlePickPlatformForSchedule = (platform) => {
    if (!schedulePlatformPicker) return;
    const [dateStr, timePart] = schedulePlatformPicker.dateTime.split('T');
    ...
    setSchedulePlatformPicker(null);
  };
  ...
  return (
    <div className="planner-page">
      ...
      {schedulePlatformPicker && (
        <div className="planner-modal-overlay" onClick={() => setSchedulePlatformPicker(null)}>
          <div className="planner-modal planner-modal--small" onClick={(e) => e.stopPropagation()}>
            <h3>Schedule post</h3>
            <p className="planner-modal-hint">Pick a platform to create your post</p>
            <div className="planner-modal-platforms-grid">
              {activeIntegrations.map((i) => (
                <button
                  key={i.id}
                  type="button"
                  className="planner-modal-platform-card"
                  onClick={() => handlePickPlatformForSchedule(i.platform)}
                >
                  {i.platform}
                </button>
              ))}
            ...
            <button className="planner-modal-cancel" onClick={() => setSchedulePlatformPicker(null)}>
              Cancel
            </button>
          </div>
        </div>
